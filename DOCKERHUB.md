...
docker run --rm -it \
  -v "$PWD:/workspace" \
  -w /workspace \
  terrenefoundation/kailash-coc-py:1.23.0
```
...
| Tag      | Meaning                                                                                 |
| -------- | --------------------------------------------------------------------------------------- |
| `1.23.0` | Current stable. Pinned semantic version — recommended for derivative builds.            |
| `latest` | Moving pointer to the most recent stable release.                                       |
| `1.22.0` | Prior stable (2026-08-14). Retained for provenance.                                     |
| `1.21.0` | Prior stable (2026-08-04). Retained for provenance.                                     |
| `1.10.1` | Prior stable (2026-05-31). Retained for provenance.                                     |
| `0.1.0`  | First-cut release (2026-05-28). Retained for provenance; prefer `1.23.0+` for new work. |
...
FROM terrenefoundation/kailash-coc-py:1.23.0@sha256:<digest>
...
```bash
docker manifest inspect terrenefoundation/kailash-coc-py:1.23.0
```
...
```bash
docker run --rm terrenefoundation/kailash-coc-py:1.23.0 bash -c '
  claude --version
  codex --version
  gemini --version
