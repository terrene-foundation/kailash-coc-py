...
Below the `Delegate` API sits `kaizen.llm.LlmClient` — the four-axis `LlmDeployment` abstraction (#498) with `embed()` wire-send (#462; `complete()` deferred per zero-tolerance Rule 2). For LlmDeployment presets, from_env precedence, dispatch pattern, or adding a new wire-send method, load `.codex/skills/04-kaizen/kaizen-llm-deployment.md` first. Authoritative spec: `specs/kaizen-llm-deployments.md`. Cross-SDK parity: `esperie-enterprise/kailash-rs#406` + `#393` + `#394`.

## Full Documentation

- `.codex/skills/04-kaizen/SKILL.md` -- Complete Kaizen skill index
- `.codex/skills/04-kaizen/kaizen-advanced-patterns.md` -- Advanced patterns
- `.codex/skills/04-kaizen/kaizen-llm-deployment.md` -- LLM wire layer (#498 + #462)
