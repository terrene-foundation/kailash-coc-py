...
function isNeverSynced(relPath, base, segs) {
  ...
  // `exclude: sync-manifest.yaml` rule, implemented as a plain `cp` at
  // coc-sync Step 4.6. That copy bypasses the tier-lane copy loop, so
  // `stripBuildInternalReferences` NEVER runs on it: the manifest reaches
